...
describe('DiamondTest', async function () {
  ...
  // New test cases
  it('should fail when trying to add duplicate function selectors', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const test1Facet = await Test1Facet.deploy()
    ...
    try {
      ...
      assert.include(error.message, 'reverted with custom error')
    }
  })

  it('should fail when trying to remove non-existent function selectors', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
  ...
  it('should fail when trying to replace non-existent function selectors', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const test1Facet = await Test1Facet.deploy()
    ...
    try {
      ...
      assert.include(error.message, 'FunctionDoesNotExist')
    }
  })
  ...
  it('should verify ownership transfer', async () => {
    ...
    assert.equal(currentOwner, newOwner.address)
  })

  it('should fail when trying to use incorrect facet cut action', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const test1Facet = await Test1Facet.deploy()
  ...
  it('should fail when trying to add functions with empty selectors', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const test1Facet = await Test1Facet.deploy()
    ...
    try {
      tx = await diamondCutFacet.diamondCut(
        [{
          facetAddress: test1Facet.address,
          action: FacetCutAction.Add,
          functionSelectors: [] // Empty selectors array
        }],
        ethers.constants.AddressZero, '0x', { gasLimit: 800000 })
      await tx.wait()
      assert.fail('Should have thrown an error')
  ...
  it('should fail when trying to add functions with zero address facet', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const test1Facet = await Test1Facet.deploy()
    ...
    try {
      tx = await diamondCutFacet.diamondCut(
        [{
          facetAddress: ethers.constants.AddressZero, // Zero address
          action: FacetCutAction.Add,
          functionSelectors: selectors
        }],
        ethers.constants.AddressZero, '0x', { gasLimit: 800000 })
  ...
  it('should fail on initialization errors', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const test1Facet = await Test1Facet.deploy()
    ...
    try {
      tx = await diamondCutFacet.diamondCut(
        [],
        ethers.constants.AddressZero, '0x1234' // Init address is zero but calldata not empty
      )
      await tx.wait()
      assert.fail('Should have thrown an error')
    } catch (error) {
    ...
    try {
      tx = await diamondCutFacet.diamondCut(
        [],
        test1Facet.address, '0x' // Init address not zero but calldata empty
      )
      await tx.wait()
      assert.fail('Should have thrown an error')
    } catch (error) {
  ...
  it('should fail when facet has no code', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const noCodeAddress = "0x1234567890123456789012345678901234567890"
    try {
      tx = await diamondCutFacet.diamondCut(
        [{
          facetAddress: noCodeAddress,
          action: FacetCutAction.Add,
  ...
  it('should fail when trying to remove immutable functions', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    try {
      tx = await diamondCutFacet.diamondCut(
        [{
          facetAddress: ethers.constants.AddressZero,
          action: FacetCutAction.Remove,
          functionSelectors: ['0x1f931c1c'] // diamondCut function selector
        }],
        ethers.constants.AddressZero, '0x', { gasLimit: 800000 })
      await tx.wait()
      assert.fail('Should have thrown an error')
  ...
  it('should fail when init function reverts', async () => {
    const [owner] = await ethers.getSigners()
    diamondCutFacet = diamondCutFacet.connect(owner)
    const Test1Facet = await ethers.getContractFactory('Test1Facet')
    const test1Facet = await Test1Facet.deploy()
    ...
    try {
      tx = await diamondCutFacet.diamondCut(
        [],
        test1Facet.address, '0x12345678' // Invalid calldata that will cause reversion
      )
      await tx.wait()
      assert.fail('Should have thrown an error')
    } catch (error) {
